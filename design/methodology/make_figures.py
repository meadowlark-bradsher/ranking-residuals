...
Data comes from evidence/evidence.json -- the single source of truth for every
quantity cited in the papers, regenerated by evidence/generate.py and checked by
evidence/verify.py. Figures derive from it; they never carry numbers of their own.
Styling is deliberately quiet: serif to match the document, one accent, no
chartjunk.
"""
import json, sys
...
def fig_rho_and_plateau():
    ...
    R = V("rho-tradeoff"); rho = [r["rho"] for r in R]
    b = [r["bias_pct"] for r in R]; se = [r["se_pct"] for r in R]
    ...
    a1b = a1.twinx()
    a1b.plot(rho, [r["unfittable"] for r in R], "s--", color=BAD, ms=3.5, lw=1.1,
