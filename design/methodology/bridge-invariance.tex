...
\begin{remark}[What the bound does and does not license]
...
\begin{proposition}[Gradient families are invisible under re-instantiation]
...
pointwise, so $\operatorname{range}\Cov(B)\subseteq\im D_0$ and, by Lemma~1,
\[
...
is the one limit that separates quenched disorder from signal (Remark~4), and it
...
resampling available. Measured over $2000$ independent fabricators, each placing
the complex items at its own levels: harmonic energy $10.000000$, standard
deviation $2.6\times10^{-13}$.
...
$\mathbb{E}[B]=-(D_0s)|_{E_b}$, Corollary~1 gives
\begin{equation}
