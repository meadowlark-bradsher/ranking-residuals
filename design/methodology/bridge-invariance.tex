...
\]
for all $v$. The $m$ circle vertices form one $r$-orbit, so the divergence is constant on them; it is zero on integer vertices, which $Y_{cc}$ does not touch. As $\sum_v(D_0^{\top}Y)_v=0$ identically, the constant vanishes. For (c), the group acts on $E_b$ as well, and the first draft of this proof missed it: the circle's generator $r$ sends the bridge edge $(i,c_j)$ to $(i,c_{j+1})$, so equivariance is not vacuous there. It forces $B(i,c_j)=B(i,c_{j+1})$ for every $j$ --- $B$ constant in the circle index --- a family of dimension $|V_{\Z}|$, not $|E_b|$. What no group element does is relate a bridge edge to an edge of another type, so nothing in the symmetry ties that family to $D_0s$ or to $Y_{cc}$, and both Hodge types occur inside it: $B(i,\cdot)=c-s_i$ is $D_0\psi$ for $\psi=(s,c)$ and is a gradient, while $B\equiv c$ would need $\psi(c_j)-\psi(i)=c$ for all $i$, forcing $\psi$ constant on $V_{\Z}$, which $s$ is not. The family is positive-dimensional and symmetry does not choose its Hodge type, which is the claim.
\end{proof}

The moral of Proposition~\ref{prop:hodgetype} is that the part of the construction that ``does not fit in pure mathematics'' fits exactly: it is the block the symmetry group acts on without relating it to either population, so equivariance can shape a bridge flow but cannot choose its Hodge type. A bridge that emerged canonical would contradict (c)---it would present two symmetry-unrelated orbits as related, i.e. present incomparability as a hidden order.
