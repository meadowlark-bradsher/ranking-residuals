...
reports the \emph{harmonic ranking residual}: the globally cyclic component of
the comparison ``flow'', what remains once the gradient a scalar ordering explains
and the curl a triangle-local check reports have both been removed --- a
one-sided reading,
sufficient for non-rankability but not equivalent to it. Used as a decision procedure it needs a threshold, and the threshold
...
\paragraph{Motivating application.} Pairwise AI evaluators are the setting this
was built for. A language-model judge may compare candidates consistently on
individual pairs while the collection of those judgements admits no coherent
scalar ordering, and that failure has two shapes: local, appearing inside filled
comparison triads, and global, appearing only around holes in the comparison
complex. The quantity studied here is the second --- the \emph{harmonic ranking
residual}, what is left of the ranking residual once gradient and triangle-local
curl structure have been separated out. Its zero is one-sided: a nonzero residual
is sufficient evidence that no scalar ordering explains the flow, while a zero
residual says nothing about the local failure, which a triad-consistency check
would already have reported. The aim here is metrological rather than normative
--- to establish when that residual is being measured correctly, before asking
what it implies about a deployed judge.

Pairwise comparison models arise in AI evaluation, preference learning, and
psychometric measurement. In each setting, a scalar latent score explains only
the gradient component of the observed comparison flow. The present work
concerns the residual structure that remains, and specifically its harmonic
component: globally inconsistent comparison structure not attributable to filled
local cycles.
...
spurious harmonic mass. On the complete graph the amount is not merely nonzero
but available in closed form.

\begin{observation}[The sign artefact is $\tfrac{n-2}{3n}$]\label{obs:pm1}
On $K_n$ under the empty filling, the $\pm 1$ flow of a total order carries
\begin{equation*}
  \grad = \frac{2(n+1)}{3n},
  \qquad
  \curl = 0,
  \qquad
  \harm = \frac{n-2}{3n} \;\longrightarrow\; \frac{1}{3} .
\end{equation*}
\end{observation}

Index the items in rank order and the flow is the all-ones edge vector
$\mathbf{1}$. The normal equations for $\min_s \sum_{i<j}(s_j - s_i - 1)^2$ read
$n\,s_v = 2v - (n-1)$ in the zero-sum gauge, so the fitted potential is affine in
the rank with slope $2/n$; since $\sum_{i<j}(j-i)^2 = n^2(n^2-1)/12$, the
gradient energy is $(n^2-1)/3$ against a total mass of $\binom{n}{2}$. The empty
filling has no $2$-cells, so what is left over is harmonic.

We checked every step against the instrument rather than only the conclusion:
the fitted potential, the gradient energy, and the fractions agree at
$n\in\{3,4,5,6,7,8,12,16\}$ within the $10^{-9}$ tolerance registered for claim
\texttt{pm1-closed-form}.

Two consequences are worth stating, because neither is visible from a pair of
measurements. The artefact is bounded --- a sign encoding cannot manufacture
arbitrary harmonic mass, only up to a third of the flow's energy --- and it is
bounded \emph{away from zero} for every $n \ge 3$, so there is no pool size at
which $\pm 1$ becomes safe. It vanishes at $n = 2$ only because $K_2$ has
$b_1 = 0$ and offers the artefact nowhere to live.

This is the third statement our specification has made about the same quantity:
an early draft quoted $0.200$ as a constant, a later one corrected it to two
measured points and the observation that the value is $n$-dependent, and it is
now the identity above. Each correction was forced by looking at one more $n$,
which is the cheapest experiment in this paper and was not run for two
revisions. Consequently every flow intended to represent a magnitude uses
value-differences or log-odds; sign encodings are used only to read a tournament
for triad statistics.
...
\bottomrule
\end{tabular}
\caption{\label{tab:oracles}Known answers and the values the instrument returns.}
\end{table}
...
\section{The null, and why it must be injected}\label{sec:null}
...
$2.7\%$ and $5.0\%$. The cross term therefore \emph{completes} the delta-method
...
floor by factors of $0.99$ to $2.30$ across the separations \S\ref{sec:guards}
admits, against $0.92$ to $1.00$ on the window --- on the graph of
...
% Deliberately NOT a float. It has no caption and no label, so its only job is to
% sit under "On one graph:" -- floating can separate it from that sentence but can
% never help it. Recomposing into combined.tex did exactly that once.
\begin{center}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\midrule
observed & $2$  & $18$  & $0.0842$ & $0.090$ \\
empty    & $20$ & $160$ & $0.0468$ & $0.090$ \\
\bottomrule
\end{tabular}
\end{center}
...
\section{Guard discipline}\label{sec:guards}
...
default separation parameter of ours that saturated the clamp --- $\beta = 0.3$,
which sits on the saturation gate that $0.25$ clears. That run's own digits are
gone, and the reason is sharper than a lost note: the first commit to carry a rig
at all already derives the window, so the incident predates the repository.
\emph{The rule survives, though,} because the estimator still takes its window as
a parameter rather than deriving it internally --- and the configuration v5
shipped with is recorded even where that run is not. Both are pinned as a
fixture.

Run at the v5 defaults, the retired rule is \emph{adequate}: it recovers the
floor at $0.98$--$1.20\times$ across every separation. Run at today's defaults it
is not. At the same $\beta = 0.3$ the floor returns at $0.88\times$ --- close
enough to read as correct, which is exactly why it passed unexamined --- while
the fitted and predicted $c$ disagree by $27\%$, which is what did not. The
derived window on that cell returns $1.09\times$, and past $\beta = 0.5$ the
retired rule degrades to $4.3\times$.

One config field separates the two arms. $n_{\mathrm{int}}$ moved from $8$ to
$12$; a larger graph carries a larger variance term; a window pinned to a
constant cannot follow it. So the fixed window was not wrong when it was
written --- it was calibrated, and then the thing it was calibrated against
moved. That is the failure \S\ref{subsec:window} records for the filling,
appearing again in a second parameter, and it is the argument for deriving the
window rather than declaring it.

The converse is the harder case, and it is why this guard is necessary without
being sufficient: it reads $c_{\mathrm{fit}}/c_{\mathrm{oracle}} = 0.95$ at a
separation where the floor is still $1.11\times$ too high. It is necessary and not
...
Not all of that movement was the seed --- the first two straddle the fit window
becoming derived (\S\ref{sec:estimation}), so the estimator moved underneath them
--- but each was a point quoted from a single draw, and each \emph{looked} settled
because nothing in the output marked it as variable. Re-measured over $20$
independent base seeds at the configuration then current ($\rho = 3.0$, grid to
$4096$), the residual was $1.6\% \pm 0.2\%$ and coverage \emph{typically}
$13/16$, so ``15/16'' had been the best of twenty draws. At the shipped
configuration ($\rho = 1.5$, grid to $16384$; Table~\ref{tab:rho}) the same two
quantities are $+0.43\% \pm 0.09\%$ and a median of $15/16$, which makes the
point a second time: neither figure survives being quoted without the
...
run supports almost any of them. The same quantity computed with exact energies
is $+0.3635\% \pm 0.0020\%$, a per-draw range of $0.034$ pt against the $1.685$
pt plotted here. The first two figures straddle the fit window becoming derived,
so part of their separation is the estimator; the rest is the sampling
law.\label{fig:draws}}
\end{figure}
...
exercises it against the real instrument. The instrument's acceptance suite is $70$
tests running in seconds, fast enough on every edit; the repository carries
further suites for work built on the instrument, which are not part of this
acceptance. Three features are deliberate.
...
The instrument reproduces every known answer (Table~\ref{tab:oracles}). The synthetic adversarial
...
harmonic \emph{fraction} over the same sweep moves only $0.0008$ to $0.0047$,
...
$4$-cycle beside a transitive triangle, the coefficient of
consistency~\cite{kendall1940} sees only the triple it can, finds it perfectly
transitive, and reports $1.0$ --- while a third of the flow's energy is harmonic
and unrankable.
...
$(-0.7\%, +3.4\%)$ to $(-0.52\%, +1.16\%)$. These are \emph{sampled} figures at
$\texttt{reps}=16$, which is what the two levers were tuned against; the same
quantity on exact energies is $+0.3635\% \pm 0.0020\%$, and the text below
accounts for the difference.\label{tab:rho}}
\end{table}
...
(\Ph)_{ee}\,v_2(p_e)$, and on both calibration topologies the composition is
$88$--$95\%$ the third term, with $\langle b,\Ph b\rangle$ contributing
$0.6$--$2.5\%$. The expectation that whatever
...
residual is $+0.3635\% \pm 0.0020\%$. Two consequences follow. First, the shipped
...
$+0.0786\% \pm 0.0008\%$; fitting $c_2$ as a third parameter moves it to
$-0.0084\% \pm 0.0004\%$; and changing the edge estimator moves the residual in
...
outright --- giving $22.75\%$ of the raw $c_2$ on this topology and $22.88\%$ of
the raw residual, agreement to under $0.2$ pp. That estimator is used here only as
...
$\Ph$-weighted edge-probability mix --- $13.5\%$ on a mid-range topology against
$22.7\%$ on one whose edges reach $p\approx0.07$. The mechanism is universal; its
...
Stated as a chain, this document is one stage of several: pairwise judgements,
then the structural decomposition and its calibration, which is the work here;
then a statistical test of the harmonic residual against a matched null; then
whatever stability and external-validity work a deployment demands. The intended
application is not to certify that a judge is aligned with an evaluation
criterion, but to make one prerequisite to that assessment measurable --- whether
its pairwise judgements carry global inconsistencies that cannot be attributed to
finite sampling or to localised cyclic disagreement. That is structural auditing,
and every normative question sits downstream of it.
...
criterion is rankable before testing whether it is; a null fitted to the
gradient-and-curl subspace does not. Choosing that subspace is part of generating
the matched null Principle~\ref{prin:nouniversal} already requires, and it has to
be stated rather than defaulted.

We have since made that choice and measured its consequences, and report them
separately. Two findings bear on the scope of this document. First, the cost of
defaulting to the gradient is not the misstatement of degree this paragraph
originally implied: fed flows that satisfy a gradient-and-curl null exactly, a
Bradley--Terry fit rejects \emph{every} draw once the curl fraction reaches
$0.45$, while the subspace null holds nominal size. Curl-type misspecification
does not bias such a test, it destroys it. Second, the subspace is not free to
choose independently of the filling: with no $2$-cells the curl space is trivial
(\S\ref{sec:problem}), the two nulls coincide, and the distinction this
paragraph draws disappears. The choice of filling and the choice of null are one
decision, not two --- which is Principle~\ref{prin:nouniversal} again, reaching a
quantity we did not anticipate it would reach.

\subsection{Future work}

The residual calibrated here is one of three, and the three are different
failures. A comparison flow that admits no single scalar order can fail
\emph{locally}, as curl --- which a triad-consistency check reports, and which
\S\ref{sec:problem} keeps as a separate diagnostic, since a zero harmonic
reading does not exclude it; \emph{globally}, as the harmonic residual this
document calibrates; or \emph{not at all within any one instantiation} while
failing across them --- a comparator whose judgements are a coherent gradient
every time and a different gradient each time, invisible to any harmonic reading
and visible only to the dispersion of the fitted gradient across
re-instantiations. A fuller structural audit would characterise all three
jointly: the curl residual, the harmonic residual, and cross-instantiation
gradient stability. Only the second has a matched null of the kind built here.
The first has classical diagnostics but no calibration against a null that knows
its own topology; the third has, so far, only the argument that it is needed.

The immediate motivating case is a language-model comparator, but the measurement
problem is not AI-specific. Bradley--Terry~\cite{bradley1952} and related
paired-comparison generalised linear models posit a scalar latent ordering, while
comparative psychometric models make closely related measurement assumptions. The
calibrated harmonic residual therefore suggests a general diagnostic for
departures from scalar comparative structure. Whether the same machinery is
useful for item-response-theory model checking, test linking, or scale drift is
an empirical question left for future work.

The present calibration treats the comparison complex as fixed independently of
the measured flow. In adaptive comparison systems this assumption fails: earlier
judgements may determine which comparisons are collected later, making the
eventual projector $\Ph$ itself data-dependent. This raises both a
selective-inference problem --- when loops or residual directions are discovered
from the same data on which they are tested --- and a design problem --- how to
choose comparisons so that local and global ranking residuals remain
identifiable. Recent orthogonalization approaches to data-dependent hypothesis
testing~\cite{dharamshi2026} offer one possible route for the former ---
they reach post-selection inference without pre-specifying the selection
mechanism, which is the shape the adaptive case takes here. Outcome-adaptive
acquisition requires separate treatment.
...
\bibitem{dharamshi2026}
A.~Dharamshi, R.~Zou, and D.~Witten.
\newblock Testing hypotheses via orthogonalization.
\newblock arXiv:2606.29732, 2026.
\end{thebibliography}
