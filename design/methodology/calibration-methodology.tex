...
$b_1 = |E| - \rank D_0 - \rank D_1$, the number of independent holes.
...
\subsection{The bridge must be potential-consistent}
...
\label{eq:coracle}
\end{equation}
which gives an independent check on the fit. Model \eqref{eq:model} is linear in
...
\subsection{The fit window, and why it is derived}
...
floor by factors of $0.83$ to $2.48$ depending on separation. Adding an explicit
$c_2/k^2$ term is worse, not better --- it is poorly conditioned and eats the
intercept. Discarding the contaminated points works.
...
\label{eq:window}
\end{equation}
where $\rho$ is a resolvability margin: at $k_{\min}$ the variance term has
fallen to $1/\rho$ of the floor being resolved. A grid that cannot reach
...
would have licensed exactly the errors it cannot see.
...
exercises it against the real instrument. The current suite is $62$ tests in
$2.4$ seconds, fast enough to run on every edit. Three features are deliberate.
...
A bias of $+0.43\% \pm 0.09\%$ survives both levers. It remains statistically
real ($t=5.0$) but is now roughly a third of the per-cell sampling spread ---
below the resolution at which any single sweep can see it. Neither margin nor
grid length removes it, so closing it further would require a different
mechanism. We note its direction: the floor is under-read, so a threshold
calibrated from it sits slightly low, biasing toward false alarms rather than
missed detections. For an instrument whose purpose is detecting non-rankability,
that is the conservative direction in which to be wrong.

\section{Scope: the threshold is topology-bound}

The most consequential limit the method establishes is not a defect but a fact
about the object.

\begin{observation}[$b_1$ is a property of the graph, not the item count]
Under the observed filling at edge retention $0.45$, the fraction of graphs with
$b_1 = 0$ runs $28.8\%$ at $n=8$, $10.8\%$ at $n=12$, and $11.9\%$ at $n=14$. It
plateaus; adding items does not drive it to zero.
\end{observation}

The same fact appeared independently as the filling dependence of the fit window
(\S5.3), where a constant calibrated under one topology under-read the floor by
$6\times$ under another. These are one object. Since $\Ph$ is determined by the
comparison graph, so are the floor, the variance coefficient $c$, and any
threshold derived from them.

\begin{principle}[No universal threshold]
...
The derived window of \S5.3 is the template for this at one level down. The
