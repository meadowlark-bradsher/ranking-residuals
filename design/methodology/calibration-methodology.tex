...
p_e)$ also carries a mean bias $b_e=(2p_e-1)/\bigl(2p_e(1-p_e)\bigr)$ at order
$k^{-1}$, and expanding $\|\Ph\mathbb{E}[Y]\|^2$ rather than
$\mathbb{E}\|\Ph Y\|^2$ leaves a cross term
$2\varepsilon\langle h,\Ph b\rangle/k$ beside it. Measured on one graph, adding
it moves the fitted-to-predicted ratio from $1.251$, $1.296$, $1.404$ to
$1.194$, $1.196$, $1.244$ at $\varepsilon=0.2,0.3,0.4$ --- not closing the gap,
but flattening its drift in $\varepsilon$, which is the part that made the check
hard to read. The residual disagreement is a further $O(k^{-1})$ contribution not
yet accounted for. We keep \eqref{eq:coracle} as the shipped predictor, since it
is what the guard was calibrated against, and record the cross term as the
identified next term rather than an unexplained constant. Model \eqref{eq:model} is linear in
...
run supports almost any of them.\label{fig:draws}}
\end{figure}
