...
gone: it predates both the derived window and the change of default, and its
configuration was never recorded. \emph{The rule survives, though,} because the
estimator still takes its window as a parameter rather than deriving it
internally --- so the retired rule is one argument away and is kept as a fixture.
Pinned back to $k \geq 64$ on the present rig, it reproduces the incident's shape
at the same $\beta = 0.3$: the floor returns at $0.88\times$ the oracle, close
...
derived window on that same cell returns $1.09\times$. Past $\beta = 0.5$ the
retired rule is worse at every separation, reaching $4.3\times$ at $\beta = 0.7$,
where the guard reads $0.27$ and is impossible to miss.
