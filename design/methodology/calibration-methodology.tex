...
which sits on the saturation gate that $0.25$ clears. The floor it returned was
an entirely plausible number, close enough to the oracle to pass unexamined, and
only the disagreement between the fitted and the predicted $c$ revealed the fit
as misspecified rather than informative. \emph{The digits from that incident are
not quoted here.} They were measured before the window became derived and before
the default moved, so no configuration the rig can now build reproduces them, and
a number no re-run can reach is exactly what this document asks nobody to print.
The blind spot itself does reproduce: the guard reads
$c_{\mathrm{fit}}/c_{\mathrm{oracle}} = 0.95$ at a separation where the floor is
still $1.11\times$ too high. It is necessary and not
