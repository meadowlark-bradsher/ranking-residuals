...
**⚠ Sharper still (RAN-28, second pass): at the fold size the χ² approximation itself fails.** Separation is not the only thing that degrades at k/2. The first two moments of T track χ²(b₁) from k = 128 up and **do not** at k ≤ 64 — on `observed`, graph 3 reads meanT/df = 1.024 at k = 128 against 0.842 at k = 64 and 0.740 at k = 32. A k = 128 deployment that thins is therefore doing its inference at exactly the fold size where the collapse to a Rao test stops holding, so the reference distribution is wrong on top of the draws being fewer. This was masked in the earlier write-up, which classified k = 128 as good and k ≤ 32 as bad and left k = 64 unstated; the fold size is the one grid point the claim most needed to name. Thinning therefore has to clear three things, not one: exchangeability, per-graph separation, and χ² validity at k/2.

**Precondition (auditable, not a leap):** thinning requires within-edge exchangeability — no position drift, criterion mixing, or time trend inside an edge. Position and criterion are in the seam, so this is an auditable check. It is the **first of three** gates on whether thinning-replaces-fission is real; the second is the per-graph separation measurement at fold size, and the third is χ² validity at that fold size (see the two corrections above). A clean audit alone does not greenlight thinning.
