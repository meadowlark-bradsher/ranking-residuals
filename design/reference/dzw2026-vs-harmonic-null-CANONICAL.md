...
**But the window is not b₁-independent, so the gate is NOT yet deployable.** The no-b₁-floor result above was measured with every cell pinned at saturation 0.010 — comfortably interior to a 0.02 window — and it holds there: at b₁ = 1 the variance ratio is 1.110 ± 0.027 over ten base seeds, inside the band. Re-pinning at 0.019, near the window edge, it is **3.44**. Every b₁ passes at 0.010; at 0.019 b₁ = 1 fails catastrophically while b₁ = 22 is untouched (0.965). So b₁ and saturation *interact*: b₁ does not set whether χ² holds at a given extremity, it sets **how much extremity is tolerable**, and low b₁ tolerates far less. A single 0.02 window applied across b₁ would wave through exactly the cells that fail worst. Where the b₁ = 1 boundary actually sits is bracketed between 0.010 and 0.019 and not yet located; until it is, the third gate is an open measurement rather than a free pre-check.

**Precondition (auditable, not a leap):** thinning requires within-edge exchangeability — no position drift, criterion mixing, or time trend inside an edge. Position and criterion are in the seam, so this is an auditable check. It is the **first of three** gates on whether thinning-replaces-fission is real; the second is the per-graph separation measurement at fold size, and the third is the saturation window at that fold size — closed form once the window is known, but the window is b₁-dependent and not yet pinned down (see the corrections above). A clean audit alone does not greenlight thinning.
