...
\noindent\textbf{What this is.} An on-ramp. Before working RAN-31 you need the
Rao score test to be a thing you \emph{recognize}, not decode --- because the job
is to \emph{derive} the relationship between DZW's fold and this test (expecting
them to differ at the statistic), and you cannot derive a relationship to a black
box. \textbf{There is no identity to verify:} the 2026-08-30 provenance audit
(RAN-27) found the ``fold $\equiv$ score test'' claim was never stated in the
paper or derived in the briefing. The reading is smaller and better-posed than it
was --- an afternoon of algebra with a known target, not a search for a
confirmation. Read this first; it makes DZW smaller.
...
owned, and DZW \S3.3.2 becomes a matter of \emph{deriving how their fold's
statistic differs from this one} --- and why that difference, not an identity, is
what justifies never implementing the fold in the pre-specified case.

\section{Then, and only then: DZW \S3.3.2}

You are deriving \textbf{one relationship}, and you already know how three of its
four checks come out. Instantiate DZW \S3.3.2 with $H_0$ = the subspace
constraint $\eta\in S$, the estimator = the $S$-constrained MLE, and the test
function $g$ = the harmonic directions, then line up four things as you read:
...
The provenance audit (RAN-27, 2026-08-30) and RAN-31's rewrite say checks 1,~2
and~4 \emph{pass as written} --- same null, same constrained fit, same df ($b_1$,
from $g=\Pharm$: $p=1$ per comparison, $q=b_1$). The entire question lives at
\textbf{check 3}, and the expectation to test there is \emph{divergence}:

\begin{mdframed}[style=callout]
\noindent\textbf{[RELAYED --- NOT OWNED, pending RAN-31.]} The three bullets
below are a session's derivation sketch, not a result of yours. They are what to
\emph{expect and re-derive}, not what to cite. Treating them as established would
repeat the exact error this retarget exists to correct: a paraphrase promoted to
a fact.

\begin{itemize}[leftmargin=1.4em,itemsep=0.35em,topsep=0.5em]
\item \textbf{The fold's residual is conditional.} From Algorithm~1,
  $x^{(1)}+x^{(2)}=2x$, and Proposition~2 gives the fold's moment statistic built
  on $2\bigl(x-\hat{E}_{H_0}[X\mid X^{(1)}]\bigr)$ --- the null residual
  \emph{given the noised fold}. The score residual $x-k\,\sigma(\hat\eta)$ is
  \emph{marginal}. At every finite $\Sigma$ these differ: the conditional mean
  tracks the realized data through the fold and absorbs signal (Remark~5's trap,
  deliberately dosed).
\item \textbf{The fold statistic is a function of the realized noise $W$},
  which the score test does not contain.
\item \textbf{As $\Sigma\to0$ the fold residual degenerates to zero} (the \S3.4
  power collapse) --- \emph{not} to the score residual.
\end{itemize}
\end{mdframed}

\begin{sloppypar}
If the derivation lands there, the target sentence to own cold is: \textbf{same
null, same constrained-MLE fit under $H_0$, same df under $g=\Pharm$ ---
different statistic.} In the pre-specified fixed-graph regime the classical score
test runs on full data and pays no randomization tax, so it (weakly)
\textbf{dominates} the fold; the fold's price is exactly what buys post-selection
validity. ``Skip the fold'' is justified by \textbf{dominance, not identity}.
\end{sloppypar}

\smallskip
\noindent And if the derivation lands somewhere else, \textbf{that is a finding}
--- file it on RAN-31 rather than forcing the expected answer.
...
So if you follow \S3.3.2 to the fixed-case \emph{relationship}, then hit the
$(I+B)^{-1}$ debiasing and lose the thread --- \textbf{that is a finished,
successful reading.} You will have derived what RAN-31 needs for the fixed-graph
claim and located the exact boundary:
...
\item \textbf{What are you \emph{deriving} in DZW \S3.3.2, and where do you
  stop?} \emph{(How the fold's conditional residual relates to the score test's
  marginal one --- expecting different statistics, same null/fit/df --- and why
  dominance justifies skipping the fold pre-specified; stop at the \S4
